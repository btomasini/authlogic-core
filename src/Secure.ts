...
class SecureImpl implements Secure {
    ...
    async loadFromCode(code: string, state: string | undefined) {
        ...
        const res = await axios.post(this.params.issuer + '/oauth/token',
    ...
    private async redirect(storage: Storage) {
        ...
        window.location.assign(`${this.params.issuer}/authorize?client_id=${p.clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${storage.state}&nonce=${storage.nonce}&response_type=code`);
    }
